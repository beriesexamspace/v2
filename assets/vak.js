(() => {
  ...
  function initialize() {
    ...
    function finish() {
      ...
      byId('opnieuw').classList.toggle('knop', !lastRun.wrong.length);
      byId('opnieuw').classList.toggle('knop-secundair', Boolean(lastRun.wrong.length));
    ...
    function renderComit(answers, correct) {
      ...
      let knopActie = null;
      if (!zwak.length) {
        regels.push('Alle vragen goed beantwoord.');
        ...
        const eerste = zwak[0];
        regels.push(`Oefen verder met ${eerste.chapter.naam}.`);
        if (sterk.length) regels.push(sterk.length === 1 ? 'In één hoofdstuk had je alles goed.' : `In ${sterk.length} hoofdstukken had je alles goed.`);
        const vragen = questionsFor(session.level).filter(question => question.h === eerste.chapter.id);
        if (session.mode === 'simulatie') tip = 'Probeer dit hoofdstuk in Training. Je krijgt na elke vraag meteen uitleg.';
        else if (eerste.goed / eerste.totaal < .5) tip = 'Lees eerst de theorie en oefen dit hoofdstuk daarna opnieuw.';
        else tip = 'Oefen dit hoofdstuk nog eens en neem de uitleg bij je antwoorden door.';
        knopTekst = 'Oefen dit hoofdstuk →';
      ...
      knop.hidden = !zwak.length;
      if (zwak.length) knop.setAttribute('aria-label', `Oefen ${zwak[0].chapter.naam}`);
